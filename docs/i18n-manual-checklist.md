...
**架构（双账户 + 独立标签规则）**：联系人按 `User.locale` 路由到两个独立 systeme.io 账户——`en` → 英文账户（`SYSTEME_IO_API_KEY_EN`），其他 → 中文/默认账户（`SYSTEME_IO_API_KEY`）。**每个账户有自己的触发→标签名映射**，英文账户的标签名可以和中文不同（如中文 `注册`、英文 `signup`）。英文账户某条规则**留空时，运行时回退到中文账户的对应标签名**（默认同名，配了才独立）。英文账户未配置 API Key 时，英文联系人跳过同步（不污染中文账户）。

**操作**：
1. 注册/准备一个独立的 systeme.io 英文账户（用于英文邮件序列），获取其 API Key
2. 进 `/dashboard/admin/integrations` → 「systeme.io（English 账户）」卡片 → 填入 API Key → 保存 → 点「测试连接」确认通
3. 在英文账户里创建你想要的标签（标签名可自定义，不必和中文账户一致）
4. 在英文卡片的「自动化标签规则」面板里，为各触发点选择英文账户的标签（**留空的规则会自动用中文账户的标签名**——若你想完全独立，就每条都配上）
5. 在英文账户里配置对应的英文营销邮件序列

**检查点**：
- [ ] `/dashboard/admin/integrations` 两张 systeme.io 卡片都显示「已连接」，各自测试连接通过
- [ ] 英文卡片的标签规则面板能拉取到**英文账户**的标签列表
- [ ] 你想独立命名的触发点，在英文卡片里都已配置（不想独立的可留空，走回退）
- [ ] 英文新用户注册后，联系人出现在**英文账户**并打上英文账户的标签 —— 见 C4
