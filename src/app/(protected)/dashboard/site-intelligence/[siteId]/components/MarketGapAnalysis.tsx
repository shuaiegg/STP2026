...
export function MarketGapAnalysis({ marketGap, siteId }: { marketGap: any, siteId: string }) {
    const t = useTranslations('dashboard.marketGap');
        ...
        <div className="space-y-4 pt-4 border-t border-slate-200">
            <div className="flex items-center justify-between">
                ...
                <Badge variant="muted" className="border-brand-primary/20 text-brand-primary bg-brand-primary/5">
                    {t('competitorsScanned', { count: marketGap.totalCompetitorsScanned })}
                </Badge>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="p-5 border-rose-100 bg-gradient-to-br from-white to-rose-50/30">
                    <h4 className="text-sm font-bold text-rose-900 flex items-center gap-2 mb-4">
                        ...
                        {t('competitorsHave', { count: marketGap.marketGaps.length })}
                    </h4>
                                ...
                                <div className="flex flex-col gap-1 items-start">
                                    ...
                                    {gap.intent && (
                                        <span className={`text-[9px] font-bold uppercase tracking-wider ml-1 px-1 ${selectedGap?.topic === gap.topic ? 'text-rose-500' : 'text-rose-400/80'}`}>
                                            {t(`intents.${gap.intent}` as any) || gap.intent}
                                        </span>
                                    )}
                                </div>
                                ...
                                {selectedGap?.topic !== gap.topic && (
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-[240px] z-20">
                                        <div className="bg-slate-900 text-white text-[11px] rounded-xl p-3 shadow-2xl border border-white/10">
                                            <div className="font-bold mb-1 opacity-60 uppercase text-[9px] tracking-widest">{t('tooltip.coreCompetitors')}</div>
                                            <div className="mb-2">{gap.competitors.join(', ')}</div>

                                            {gap.evidence && gap.evidence.length > 0 && (
                                                <div className="space-y-1.5">
                                                    <div className="font-bold opacity-60 uppercase text-[9px] tracking-widest">{t('tooltip.viewAnalysis')}</div>
                        ...
                        <div className="mt-6 p-5 bg-white rounded-xl border border-rose-200 shadow-inner animate-in slide-in-from-top-2">
                            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
                                <h5 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                                    ...
                                    {t('deepProfile.title', { topic: selectedGap.topic })}
                                </h5>
                            ...
                            {isLoadingProfile ? (
                                <div className="py-8 flex flex-col items-center justify-center space-y-3">
                                    ...
                                    <span className="text-xs text-slate-500 font-mono tracking-widest uppercase">{t('deepProfile.loading')}</span>
                                </div>
                            ) : gapProfile ? (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                            <div className="text-[9px] font-bold text-slate-400 tracking-widest uppercase mb-1">{t('deepProfile.contentThickness')}</div>
                                            <div className="text-lg font-black text-slate-800 tabular-nums">{gapProfile.wordCount.toLocaleString()} <span className="text-xs text-slate-500 font-medium">{t('deepProfile.words')}</span></div>
                                        </div>
                                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                            <div className="text-[9px] font-bold text-slate-400 tracking-widest uppercase mb-1">{t('deepProfile.headingStructure')}</div>
                                            <div className="text-lg font-black text-slate-800 tabular-nums">{gapProfile.headings.length} <span className="text-xs text-slate-500 font-medium">{t('deepProfile.levels')}</span></div>
                                        </div>
                                    </div>

                                    <div>
                                        <div className="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-2 flex items-center justify-between">
                                            <span>{t('deepProfile.structureBreakdown')}</span>
                                            <a href={gapProfile.url} target="_blank" rel="noopener noreferrer" className="text-brand-primary lowercase tracking-normal flex items-center gap-1 hover:underline">
                                                {t('deepProfile.originalSource')} <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" x2="21" y1="14" y2="3" /></svg>
                                            </a>
                                        </div>
                                        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 max-h-[300px] overflow-y-auto custom-scrollbar">
                                            {gapProfile.headings.length > 0 ? (
                                                ...
                                                <p className="text-slate-500 text-xs text-center py-4">{t('deepProfile.noStructure')}</p>
                                            )}
                                        </div>
                                    </div>
                ...
                <Card className="p-5 border-emerald-100 bg-gradient-to-br from-white to-emerald-50/30">
                    <h4 className="text-sm font-bold text-emerald-900 flex items-center gap-2 mb-4">
                        ...
                        {t('ourStrengths', { count: marketGap.ourStrengths.length })}
                    </h4>
                    {marketGap.ourStrengths.length > 0 ? (
                                ...
                                <div key={i} className="flex flex-col gap-1 items-start">
                                    ...
                                    {strength.intent && (
                                        <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-400/80 ml-1 px-1">
                                            {t(`intents.${strength.intent}` as any) || strength.intent}
                                        </span>
                                    )}
                                </div>
                            ))}
                        ...
                        <p className="text-sm text-emerald-600/60 font-medium">{t('noStrengths')}</p>
                    )}
                </Card>
            </div>
        </div>
