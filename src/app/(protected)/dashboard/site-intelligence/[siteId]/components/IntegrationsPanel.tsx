...
export function IntegrationsPanel({ siteId, onUpdate }: { siteId: string, onUpdate: () => void }) {
    const t = useTranslations('dashboard.integrationsPanel');
    ...
    if (!siteData) {
        return <div className="p-8 text-center text-rose-500 text-sm">{t('loadError')}</div>;
    }
                    ...
                    <div className="flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-1">{t('gscDesc')}</p>
                            </div>
                            <Badge variant="default" className={gscPropertySelected ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-200"}>
                                {gscPropertySelected ? t('statusConnected') : t('statusDisconnected')}
                            </Badge>
                        </div>
                        ...
                        <div className="mt-auto border-t border-slate-100 pt-4">
                            {!hasGscToken ? (
                                ...
                                >
                                    ...
                                    {isConnectingGSC ? t('connectingGsc') : t('buttonGscAuth')}
                                </button>
                            ) : !gscPropertySelected ? (
                                <div className="space-y-3">
                                    <p className="text-xs text-brand-primary p-2 bg-brand-primary/10 rounded">{t('authSuccessGsc')}</p>
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="text-sm font-medium text-emerald-700 bg-emerald-50 px-3 py-2 rounded flex items-center gap-2 text-wrap break-all">
                                        ...
                                        {t('mapped')} ({siteData.gscConnections![0].propertyId})
                                    </div>
                                    <button
                                        onClick={handleConnectGSC}
                                        disabled={isConnectingGSC}
                    ...
                    <div className="flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-1">{t('ga4Desc')}</p>
                            </div>
                            <Badge variant="default" className={ga4PropertySelected ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-200"}>
                                {ga4PropertySelected ? t('statusConnected') : t('statusDisconnected')}
                            </Badge>
                        </div>
                        ...
                        <div className="mt-auto border-t border-slate-100 pt-4">
                            {!hasGa4Token ? (
                                ...
                                >
                                    ...
                                    {isConnectingGA4 ? t('connectingGa4') : t('buttonGa4Auth')}
                                </button>
                            ) : !ga4PropertySelected ? (
                                <div className="space-y-3">
                                    <p className="text-xs text-orange-600 p-2 bg-orange-50 rounded">{t('authSuccessGa4')}</p>
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="text-sm font-medium text-emerald-700 bg-emerald-50 px-3 py-2 rounded flex items-center gap-2 text-wrap break-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                                        {t('mapped')} ({siteData.ga4Connections![0].propertyId.replace('properties/', '')})
                                    </div>
                                    <button
                                        onClick={handleConnectGA4}
                                        disabled={isConnectingGA4}
