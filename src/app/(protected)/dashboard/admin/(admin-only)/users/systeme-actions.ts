...
export async function createUserContact(email: string, name: string): Promise<ActionResult> {
  try {
    await requireAdmin();
...
export async function fetchUserContact(email: string, name: string): Promise<ContactResult> {
  try {
    await requireAdmin();
...
export async function addUserTag(email: string, name: string, tagId: number, tagName: string): Promise<ActionResult> {
  try {
    await requireAdmin();

    const fetchResult = await getContactByEmail(email);
    if (fetchResult.ok) {
      const res = await addTagToContact(fetchResult.contact.id, tagId);
      ...
      await addContact(email, name, [tagName]);
      // Re-fetch to verify
      const refetch = await getContactByEmail(email);
...
export async function removeUserTag(contactId: number, tagId: number, tagName: string): Promise<ActionResult> {
  try {
    await requireAdmin();
...
export async function fetchAvailableTags(): Promise<TagsResult> {
  try {
    await requireAdmin();
