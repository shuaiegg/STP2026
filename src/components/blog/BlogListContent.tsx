...
function formatDate(dateInput: Date | string | null, locale: string): string {
    if (!dateInput) return '';
    const date = typeof dateInput === 'object' && dateInput instanceof Date
        ? dateInput
        : new Date(dateInput as string);
    if (isNaN(date.getTime())) return '';
    return date.toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US', { year: 'numeric', month: '2-digit', day: '2-digit' });
}
            ...
            <div className="max-w-7xl mx-auto px-6 py-20">
                ...
                {filteredPosts.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-32 text-center">
                        ...
                        <p className="font-semibold text-brand-text-primary mb-1">{t('emptyTitle')}</p>
                        <p className="text-sm text-brand-text-muted">{t('emptyDescription')}</p>
                    </div>
                )}
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-4 text-xs text-brand-text-muted font-mono">
                                        ...
                                        {featuredPost.readingTime && (
                                            <>
                                                <span>·</span>
                                                <span>{t('minutes', { count: featuredPost.readingTime })}</span>
                                            </>
                                        )}
                                    </div>
