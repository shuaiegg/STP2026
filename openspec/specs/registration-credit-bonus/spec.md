...
The system SHALL automatically grant 5 credits to every new user upon successful registration, using better-auth's `databaseHooks.user.create.after` hook.

#### Scenario: Successful registration grants bonus
- **WHEN** a new user registers via email/OTP
- **THEN** their `User.credits` is set to 5 and a `CreditTransaction` with `type=BONUS` and `description="注册赠送"` is created within 1 second of account creation
...
- **THEN** the amount is exactly 5 credits, sufficient for one complete site audit (SITE_AUDIT_BASIC cost = 5)
...
- **THEN** the CreditTransaction record has `type="BONUS"` and `amount=5`
